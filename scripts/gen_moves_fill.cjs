...
// ── Parsers (same offsets as generation_moves.js) ──────
...
async function scrapeGen(name, gen, retries=8) {
  for (let a=1;a<=retries;a++) {
    try {
      const url = `https://wiki.52poke.com/wiki/${encodeURIComponent(name)}/第${NUM[gen]}世代招式表`;
      const r = await axios.get(url, {headers:{'User-Agent':UA,'Accept-Language':'zh-Hans'},timeout:20000});
      ...
      if (e.response?.status===429) {
        const w = Math.min(5000 * a + Math.random()*3000, 30000);
        await sleep(w); continue;
      }
      if (a<retries) { await sleep(2000*a); continue; }
      return {lv:[],tm:[],egg:[],tu:[]};
    }
  }
  return {lv:[],tm:[],egg:[],tu:[]};
...
async function fillOne(fp) {
  ...
  if (!missing.length) return {pid,name,status:'ok'};

  // Only fill Pokémon that are REASONABLE to scrape (≥2 gens to fill)
  if (missing.length === 9) return {pid,name,status:'skipped (0 existing)'};

  for (const gen of missing) {
    const g = await scrapeGen(name, gen);
    if (g.lv.length) d.learnable_moves.push(...g.lv);
  ...
  return {pid,name,status:`+${missing.length} gens`};
}
...
async function main() {
  ...
  const incomplete = files.filter(f=>{
    ...
    const gs=(d.learnable_moves||[]).filter(g=>g.generation!=null).length;
    return gs>1 && gs<6; // has some data but not complete
  });
  console.log(`Filling ${incomplete.length} partially-complete Pokémon (concurrency=2)...\n`);

  // Process with concurrency=2 to avoid 429
  let done = 0;
  for (let i=0; i<incomplete.length; i+=2) {
    const batch = incomplete.slice(i, i+2);
    const results = await Promise.allSettled(batch.map(f=>fillOne(path.join(DIR,f))));
    for (const r of results) {
      done++;
      if (r.status === 'fulfilled') {
        const s = r.value;
        console.log(`[${done}/${incomplete.length}] ${s.pid} ${s.name}: ${s.status}`);
      } else {
        console.log(`[${done}/${incomplete.length}] Error: ${r.reason?.message?.slice(0,80)}`);
      }
    }
    await sleep(200);
  }
